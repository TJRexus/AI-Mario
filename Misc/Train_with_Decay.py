...
if __name__ == "__main__":
    ...
    model = PPO('CnnPolicy', env, verbose=1, tensorboard_log="./board/", learning_rate=0.000025, n_steps=2048, batch_size=256, ent_coef=0.001)
    ...
    decay_entropy_callback = EntropyDecayCallback(
        check_freq=2048,                #Amount of timesteps until check
        log_dir=log_dir,
        initial_reward_threshold=1500,  #Initial threshold to reach
        increment=1.05,          #Increase reward by 5%
        entropy_decay_rate=0.000005,    #Decay entropy very slowly, but also quickly since the reward threshold will go up fast
        min_entropy_coef=0.000001,      #Minimum ent_coef can be to be safe
        initial_entropy_coef=0.001      #Starting ent_coef
    )
    callbacks = CallbackList([best_model_callback,no_progress_callback,decay_entropy_callback])
    model.learn(total_timesteps=500000, callback=callbacks, tb_log_name="PPO-ent001-500000") #Actually starts learning and create tensorboard
    model.save("./models/ent001")
    env.close()
    print("----------------------------Done Learning----------------------------")